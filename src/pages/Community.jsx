          ...
          {isLoading ? (
            ...
            filteredAndSorted.map((post, i) => (
              <PostCard
                ...
                index={i}
                isAdminPost={post.is_admin_post === true || adminEmails.has(post.author_email)}
                adminAvatarUrl={(post.is_admin_post === true || adminEmails.has(post.author_email)) ? (allUsers.find(u => u.email === post.author_email)?.avatar_url || post.author_avatar || null) : null}
                isAdmin={user?.role === "admin"}
              />
            ))
          )}
